				...
				<table BORDER=10 BORDERCOLOR=YELLOW>
					...
					<c:forEach items="${productList }" var="product" varStatus="status">
						<tr>
							...
							<td>
							<fmt:formatDate value="${product.mfDate }" pattern="dd/MM/yyyy"/></td>
							<td><input type="text" name="quantity${product.id }" id="quantity${product.id }" value="1" onblur="calculatePrice(${product.id })"/></td>
							<td>Rupees<input type="text" name="total${product.id }" id="total${product.id }" value="${product.price}" onblur="calculatePrice(${product.id })"/></td>
							
							<td><a
								href="ProductController?action=deleteFromCart&index=${status.index }">Remove</a>
							</td>
						</tr>
					</c:forEach>
				</table>
				
				  <button ><a href="CheckOutController?action=placeOrderToCarts">Place Order
				  </a></button>
